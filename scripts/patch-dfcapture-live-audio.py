...
# DFCapture exposes one generic stair button even though the backend already
# supports DownStair, UpDownStair and UpStair separately. A generic UpDownStair
# designation is rejected on an exposed floor, where DF requires DownStair.
# Expose all three operations so browser players can create complete stairwells.
...
# Bust browser caches for the web files modified below.
text, controls_version = re.subn(
    r'(<script src="/js/dfcapture-controls-placement\.js\?v=)([^"]+)("></script>)',
    lambda match: match.group(1) + match.group(2) + "-stairs1" + match.group(3),
    text,
    count=1,
)
...
# Sanity checks: every visible stair button must map to the backend operation
# that placement.cpp already supports.
for expected in (
